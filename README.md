...
That table is one run, on 8 September. The windows are counted back from the
current block, so they land on different days every time and the sample moves
with them. Run again on 9 September and it answers 336 proven, 327 finalized,
**9 never finalized (2.7%)**, holding 504,135 WALLET, 131,309 T, 6,025 USDT and
910 SYND, of which 7 of 9 would deliver. Two samples a day apart, two different
...
| Cannot tell whether the money will arrive | It cannot. The portal reveals a failing target call only to a caller of `address(1)`, and KeeperHub's read node has no field for who a call is from, so a workflow cannot make that call at all. The missing field is filed upstream as [keeperhub#2399](https://github.com/KeeperHub/keeperhub/issues/2399), with the two `eth_call`s that answer opposite things. `scripts/survey.mjs` runs the check instead and names why each target refuses; the keeper trusts whoever configured it. |
...
There are three, all importable JSON in `workflows/`:

- **`sweeper.json`** finds them itself. It is the one to run.
- **`on-demand-finalizer.json`** takes one withdrawal as trigger input, for
  releasing something already known.
...
All three come out of `scripts/build-workflows.mjs`, so they cannot drift apart,
and `scripts/check-live.mjs` compares what a KeeperHub account holds against
what is here. The same generator points at any OP Stack portal: the mainnet
release below was made by `on-demand-finalizer.json` built with `PORTAL` and
`NETWORK` set to Ethereum, which is the same JSON with two constants changed.

## Finding them

The two workflows above are pointed at a withdrawal somebody already found.
`sweeper.json` finds them.

The rollup's own message passer emits every field the portal later hashes, so a
query over that one event is a list of withdrawals with everything needed to
finish them. The sweeper walks that list and asks the portal about each in turn.

```
schedule tick
  -> query MessagePassed on the rollup, over a window of blocks
  -> for each withdrawal it finds
        -> the same five reads and two gates as above
        -> release it, or leave it alone
...
### It found two by itself and finished both

The sweeper, given no withdrawal and no list, over a window of two hundred
thousand Base Sepolia blocks:
...
One of the two was `0x4c74def9...`, which the survey had already flagged as
releasable. The other the survey had not shown me at all: the sweeper found it
on its own and gave it back.
...
1. Import `workflows/sweeper.json`, set `<WALLET_INTEGRATION_ID>` to your
   organization's wallet integration, and enable it. That is the whole thing:
   it reads the rollup for withdrawals, asks the portal about each, and finishes
   the ones that are ready. Nothing else has to be configured.
2. To point it at a particular withdrawal instead, import
   `workflows/on-demand-finalizer.json` and run it with seven inputs:
   `withdrawalHash`, `nonce`, `sender`, `target`, `value`, `gasLimit` and
   `withdrawalData`. `docs/example-withdrawal.json` is one, in that shape.
3. To watch one and release it the moment the portal will take it, import
   `workflows/unattended-finalizer.json`, fill the withdrawal into the two web3
   nodes, and enable it.
4. To find candidates yourself, or to look further back than one sweep:
   `node scripts/survey.mjs base-sepolia`. Take only the ones it says would
   actually deliver, and check the fields with
   `node scripts/verify-withdrawal.mjs <file>`, which recomputes the hash and
   fails on a mismatch.

For Ethereum rather than its testnet, build the workflows against the mainnet
portal and import those instead:

```
PORTAL=0x49048044D57e1C92A77f79988d21Fa8fAF74E97e NETWORK=1 L2_NETWORK=8453   node scripts/build-workflows.mjs
```
...
- A sweep is as wide as one event query. Two hundred thousand blocks answer in
  ...
  reaches further back than a sweep, and what says which withdrawals would
  deliver before you point anything at them.
- The two single-withdrawal variants carry one each, because a schedule has no
  input and there is no workflow-scoped store to keep a list in. The sweeper
  needs neither: its list is the chain.
